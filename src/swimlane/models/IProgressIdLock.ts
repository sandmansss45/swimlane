...
// lock record for this progressId+region have a blank unlockedAt?" - see
// findActiveLock.
//
// Scoped to progressId + region together, not just progressId - a
// Progress ID can hold several genuinely separate swimlanes side by side
// (see FlowRegionTabs), so locking the UK version of a flow shouldn't
// freeze the US/SA versions on the same Progress ID. region is '' when
// locked while viewing "All" (the normal case for a Progress ID that
...
// technical barrier.
export interface IProgressIdLock {
...
export function isLockActive(lock: IProgressIdLock): boolean {
  return !lock.unlockedAt;
}

/**
 * The lock currently in effect for a given swimlane (progressId + region),
...
export function findActiveLock(
  locks: IProgressIdLock[],
  progressId: string,
  region: string | undefined
): IProgressIdLock | undefined {
  const normalizedRegion = region || '';
  const matches = locks.filter(l => l.progressId === progressId && l.region === normalizedRegion && isLockActive(l));
